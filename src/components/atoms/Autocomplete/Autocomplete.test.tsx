...
describe('Autocomplete', () => {
   ...
   const setup = ({ label, suggestions, ...props }: Partial<React.ComponentProps<typeof Autocomplete>> = {}) => {
      ...
      const getClearIcon = () => autocomplete.queryByTestId("delete-icon")
   ...
   describe('item selection, clear and highlighting', () => {
      it("renders clear button on item selection", async () => {
         const onClearSelection = jest.fn();
         const { input, getClearIcon } = setup({ onClearSelection })
      ...
      it("calls onClearSelection item on clear button click", async () => {
         const onClearSelection = jest.fn();
         const { input, getClearIcon } = setup({ onClearSelection })
         ...
         expect(onClearSelection).toBeCalledTimes(1)
      });

      it("calls onObjectSelected with correct object on item click", () => {
      ...
      it("calls onObjectSelected with correct object on item selection with enter", () => {
         const onObjectSelected = jest.fn();
